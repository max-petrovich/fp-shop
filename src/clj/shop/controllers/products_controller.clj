(ns shop.controllers.products-controller
  (:use [shop.db.protocols.common]
        ...
        [shop.db.cache.products-cache-repository])
...
(defn index []
  (layout/render "product/index.html" {:products (.get-records product-repository)}))
...
(defn store [{:keys [params] :as request}]
  (let [errors (rv/validate-product-form params)]
    (if errors
      ...
      (do
        (.insert-record product-repository (merge {:user_id (:id (:identity request))
...
(defn show [id request]
  (let [row (.get-record product-repository id)]
    (if-not (empty? row)
      (do
...
(defn edit [id request]
  (if-let [product (.get-record product-repository id)]
...
(defn update [id {:keys [params] :as request}]
  (if-let [errors (rv/validate-product-form params)]
    (-> (response/found (str "/product/" id "/edit"))
        (assoc :flash (assoc params :errors errors)))
    (do
      (.update-record product-repository id (select-keys params [:category_id :price :article :title :description]))
      (-> (response/found "/")
...
(defn comments-store [product_id {:keys [params] :as request}]
  (if-let [errors (rv/validate-comments-form params)]
    ...
    (let [row (.get-record product-repository product_id)]
      (do
        (insert-record comments-repository {:product_id product_id
                                             ...
                                             :date (tc/to-sql-time (java.util.Date.))})
        ; inc comments num
        (.update-record product-repository product_id {:comments_num (+ 1 (:comments_num row))})
